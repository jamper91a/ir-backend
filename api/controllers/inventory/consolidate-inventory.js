      ...
      .transaction(async (db)=> {
        ...
        if(sails.config.custom.rawQueries) {
          let SQL = `
                SELECT
                    inventories.id,
                    inventories.zone_id as 'zone',
                    inventories.consolidated_inventory_id as 'consolidatedInventory',
                    (SELECT COUNT(1)
                        FROM inventories_has_products
                        WHERE inventories_has_products.inventory_id = inventories.id
                    ) AS products
                FROM
                     inventories
                WHERE id IN ($1)
