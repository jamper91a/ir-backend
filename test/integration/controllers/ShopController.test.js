...
describe('ShopController', function() {
  ...
  describe('#Find shop by id', function() {
    const url='/shop/find-shop-by-id';
    it('Should not allow employee', function (done) {
      request
        .post(url)
        .send({id: 0})
    ...
    it('Should not get shop that does not exits', function (done) {
      request
        .post(url)
        .send({id: 0})
        .set({Authorization: "Bearer " + sails.config.custom.tokens.admin})
        ...
        .end(function (err, res) {
          ...
          if(res.res.headers['x-exit'] === 'shopNotFound') {
            done();
          } else{
            done(new Error('Shop should no be found'));
          }
        });
    });
    it('Should not get shop from different company', function (done) {
      request
        .post(url)
